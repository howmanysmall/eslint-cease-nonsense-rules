...
const ruleTester = new RuleTester({
	...
	cachingTester.run("fast-format (cached)", cacheGuardRule, {
		invalid: [
			{
				code: "const cached=1",
				errors: [{ messageId: "REPLACE" }],
	...
	failingTester.run("fast-format (errors)", failingRule, {
		invalid: [
			{
				code: "const broke=1",
	...
	failingTester.run("fast-format (errors, cached)", failingRule, {
		invalid: [
			{
				code: "const broke=1",
				errors: [{ message: "Oxfmt error: formatter broke" }],
