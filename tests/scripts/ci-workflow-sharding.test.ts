...
describe("ci-workflow-sharding", () => {
	...
	it("test-heavy matrix file section contains all manifest heavy files", () => {
		...
		for (const file of HEAVY_FILES) {
			expect(CI_YAML).toContain(file);
		}
	});
});

describe("release-workflow-sharding", () => {
	it("release test and test-heavy jobs keep needs: validate-and-build", () => {
		expect.assertions(1);
		const dependencyCount = [...RELEASE_YAML.matchAll(/needs:\s+validate-and-build/gu)].length;
		expect(dependencyCount).toBeGreaterThanOrEqual(2);
	});

	it("release old grep filter blocking orphaned test files is removed", () => {
		expect.assertions(1);
		expect(RELEASE_YAML).not.toContain("grep -v -E");
	});

	it("release normal shard step uses the --normal-lane manifest CLI flag", () => {
		expect.assertions(1);
		expect(RELEASE_YAML).toContain("--normal-lane");
	});

	it("release test-heavy matrix file section contains all manifest heavy files", () => {
		expect.hasAssertions();
		for (const file of HEAVY_FILES) {
			expect(RELEASE_YAML).toContain(file);
		}
	});

	it("release test-heavy case_shard matrix remains [1, 2]", () => {
		expect.assertions(1);
		expect(RELEASE_YAML).toMatch(/case_shard:\s+\[1,\s*2\]/u);
	});
});
