...
badges:
    ...
    - text: "Suggestion"
      variant: note
description:
    Flags useEffect that only moves data around inside React (derived state, resets, parent notifications, event flags,
    duplicates, and chains).
---
...
Flags `useEffect` calls that only move data around inside React. If the logic can run during render or in an event
handler, it usually shouldn't be an effect.

## Rule Details :badge[Best Practice]{variant=success}

Reports these patterns (each can be toggled):
...
All `report*` options default to `true` unless you turn them off.
