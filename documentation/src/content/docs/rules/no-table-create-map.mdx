...
badges:
    ...
    - text: "Suggestion"
      variant: note
description: Disallow calling map() directly on table.create(...) and new Array(...).
---
...
Reports constructor chains where `map` is called immediately after `table.create(...)` or `new Array(...)`. In
roblox-ts, this hides fill semantics and makes the code harder to follow.
...
<Code
	code={["{", '\t"cease-nonsense/no-table-create-map": "error"', "}"].join("\n")}
	lang="typescript"
