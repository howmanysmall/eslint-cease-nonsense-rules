...
React requires `key` props on elements in arrays to efficiently track changes. This rule enforces keys on all JSX elements except the root return of a component, preventing common React warnings, state bugs, and performance issues.
...
2. **Select a Stable ID**
   Choose a property that is unique and persistent (like a database ID or UUID). Avoid using the array index.

3. **Apply the Key**
   Add the `key` prop to the outermost element returned for each item.

4. **Handle Fragments**
   If returning multiple elements per item, use a keyed `<Fragment>` instead of the short `<>` syntax.
...
<CardGrid>
	...
	/>
	<LinkCard title="ban-react-fc" description="Enforces proper component patterns" href="/eslint-cease-nonsense-rules/rules/ban-react-fc/" />
</CardGrid>
