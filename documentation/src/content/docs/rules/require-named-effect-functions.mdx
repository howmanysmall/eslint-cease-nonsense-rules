...
Inline arrow functions in hooks like `useEffect` are convenient but problematic. Stack traces show them as
`(anonymous)`, DevTools cannot label them helpfully, and complex logic becomes hard to read. This rule requires that
effect callbacks have a name, either as a separate function definition or a named function expression.
