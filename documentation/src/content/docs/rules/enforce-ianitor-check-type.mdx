...
Require `Ianitor.Check<T>` validators for complex types.

## Rule Details :badge[Roblox]{variant=note}

This rule calculates the structural complexity of your TypeScript types and requires `Ianitor` validators once they
cross a configured threshold. Runtime validation matters most at boundaries like DataStores and RemoteEvents, where
TypeScript can't prove the data is shaped correctly.

<Aside type="caution" title="Runtime Safety">
	TypeScript can't validate runtime data. `Ianitor` can.
</Aside>
...
<CardGrid>
	...
	<Card title="Nesting Depth" icon="magnifier">
		Deep nesting drives complexity up quickly.
	</Card>
	...
	<Card title="Performance" icon="setting">
		Uses caching so the complexity check stays cheap.
	</Card>
</CardGrid>
