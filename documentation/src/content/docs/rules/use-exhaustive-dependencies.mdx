...
badges:
    ...
    - text: "Fixable"
      variant: success
description: Make hook dependency arrays match what the callback actually uses.
---
...
Make dependency arrays match what the callback uses.

## Rule Details :badge[Best Practice]{variant=success}

If you use a value from component scope inside a hook callback, it belongs in the dependency array. Missing deps lead to
stale closures. Extra deps lead to unnecessary reruns.

<Aside type="tip" title="Why it matters">
	Stale closures are an easy way to ship a bug in React. Let the linter do the bookkeeping.
</Aside>
