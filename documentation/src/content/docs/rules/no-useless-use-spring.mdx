...
badge:
    text: Performance
    variant: caution
description: Flags useSpring-style hooks that never change due to static configuration and dependencies.
---

import { Aside, Code, LinkCard, CardGrid, Card, Tabs, TabItem, Badge, Steps } from "@astrojs/starlight/components";

Flags `useSpring`-style hooks that never change.

## Rule Details :badge[Performance]{variant=caution}

Spring hooks (`useSpring`, `useMotion`, etc.) carry significant runtime overhead in `roblox-ts`. If both the
configuration and the dependencies are entirely static, the spring value will never update. In these cases, the hook is
totally redundant and should be replaced with a simple constant or state value.

<Aside type="danger" title="Unnecessary Subscriptions">
	Every `useSpring` call creates reactive subscriptions and internal update loops. Using them for static values wastes
	CPU cycles and memory for zero visual result.
</Aside>
...
<CardGrid>
	<LinkCard
		...
		description="Ensure corect react dependencies"
		href="/eslint-cease-nonsense-rules/rules/use-exhaustive-dependencies/"
	/>
	<LinkCard title="no-identity-map" description="Bans wasteful data transformations" href="/eslint-cease-nonsense-rules/rules/no-identity-map/" />
</CardGrid>
