...
badges:
    - text: "Suggestion"
      variant: note
description: Prefer early returns over wrapping the whole function body in an if.
---
...
If a function is wrapped almost entirely in a single `if`, flip the condition and return early instead. It keeps the
main path at the top level and avoids pointless indentation.

<Aside type="note" title="Cognitive Load">
	Deep nesting makes code harder to scan. Guard clauses keep functions flat.
</Aside>
