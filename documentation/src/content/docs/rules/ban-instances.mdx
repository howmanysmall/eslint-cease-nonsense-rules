...
badges:
    - text: "Roblox"
      variant: caution
description: Ban specific Roblox Instances in new Instance() and JSX (configurable).
---
...
Stops you from creating specific Roblox Instance classes. Configure the banned list and (optionally) attach a short
reason so the error message tells people what to use instead.

<Aside type="note" title="Architectural Control">
	Use this rule to enforce project-level choices, like banning `Part` in favor of `MeshPart` or disallowing runtime
	`Script` creation.
</Aside>
