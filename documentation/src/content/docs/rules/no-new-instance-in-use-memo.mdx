...
`useMemo` should stay pure. Constructing objects inside the callback (especially runtime resources like Roblox Instances)
mixes allocation and lifecycle work into memoization, which makes component behavior harder to reason about.

<Aside type="caution" title="Render-Phase Allocation">
	If you need to create and clean up runtime objects, prefer explicit lifecycle management (`useEffect`, refs, or module
	scope factories) over constructor calls inside `useMemo`.
</Aside>
