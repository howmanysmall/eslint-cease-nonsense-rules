...
Prefer combining multiple `world.get()` or `world.has()` calls on the same entity.

## Rule Details :badge[Performance]{variant=success}

In Jecs (a Roblox ECS library), calling `world.get()` or `world.has()` multiple times on the same entity triggers
multiple archetype lookups. Combining these calls reduces lookups and cache misses. This rule detects consecutive
lookups on the same entity and merges them.
...
<RuleExample type="fail" title="Incorrect">
		### Redundant Lookups
		Separate calls on the same entity force extra lookups.
...
<RuleExample type="pass" title="Correct">
    	### Grouped Queries
    	Use the multi-argument overloads to keep it to one lookup.
...
<CardGrid stagger>
	...
	<Card title="Jecs Limits" icon="setting">
		Respects Jecs limits (up to 4 components) by only merging valid counts.
	</Card>
</CardGrid>
