...
badge:
    text: Roblox
    variant: note
description: Disallow append assignment patterns like array[array.size()] = value in roblox-ts code.
---

import { Aside, Code, LinkCard, CardGrid, Card, Tabs, TabItem, Steps } from "@astrojs/starlight/components";

Disallow append-style writes that use `.size()` as an index in assignment expressions.

## Rule Details :badge[Roblox]{variant=note}

In roblox-ts code, `array[array.size()] = value` is an append pattern that is easy to miss during review and harder to
search for. This rule enforces explicit append semantics and reports these assignments.

<Aside type="tip" title="Clear Append Intent">
	Using `array.push(value)` makes append intent obvious and avoids index math in write sites.
</Aside>
