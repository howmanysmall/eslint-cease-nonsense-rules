...
3. **Type Props Directly** Move the prop type from the variable annotation to the function parameter list:
   `(props: Props)`.
4. **Clean Up** If your component accepts `children`, move that into your shared wrapper type such as
   `PropertiesWithChildren` instead of manually declaring `children` on the props interface.
{/* oxfmt-ignore */}
