...
<Aside type="tip" title="When to use this rule">
	Use this rule when you want to catch accidental constant conditions in `if` statements and loops without a loop
	exit, while allowing intentional infinite loops with proper exit mechanisms.
</Aside>
...
<CardGrid stagger>
	...
	<Card title="Exit Path Analysis" icon="target">
		Traces loop exits (`break`, `return`) and configurable exit calls, and verifies labeled breaks target the same
		loop.
	</Card>
